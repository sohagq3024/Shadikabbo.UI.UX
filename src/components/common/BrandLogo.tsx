...
export const BrandLogo: React.FC<BrandLogoProps> = ({
  ...
  // Generate unique ID per instance to prevent SVG gradient/def collisions across multiple elements
  const reactId = useId();
  const uid = `logo_${reactId.replace(/[^a-zA-Z0-9_-]/g, '_')}`;
  ...
  // The custom vector mark matching the stylized bride & groom motif
  const LogoIcon = (
    <svg
    ...
    >
      <defs>
        <linearGradient id={`${uid}_groomGrad`} x1="20" y1="20" x2="60" y2="100" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#253582" />
          <stop offset="50%" stopColor="#1B2668" />
          <stop offset="100%" stopColor="#121A45" />
        </linearGradient>
        <linearGradient id={`${uid}_brideGrad`} x1="50" y1="30" x2="90" y2="105" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#FF2A42" />
          <stop offset="60%" stopColor="#D91B2B" />
          <stop offset="100%" stopColor="#A8101E" />
        </linearGradient>
        <linearGradient id={`${uid}_swooshRed`} x1="10" y1="65" x2="70" y2="105" gradientUnits="userSpaceOnUse">
          <stop offset="0%" stopColor="#EA2536" />
          <stop offset="100%" stopColor="#B30E1D" />
        </linearGradient>
        <linearGradient id={`${uid}_veilGrad`} x1="65" y1="35" x2="95" y2="70" gradientUnits="userSpaceOnUse">
      ...
      <g>
        ...
        {/* Groom Head */}
        <circle cx="39" cy="24" r="9" fill={`url(#${uid}_groomGrad)`} />
        ...
        <path
          ...
          fill={`url(#${uid}_groomGrad)`}
        />

        {/* Bride Head */}
        <circle cx="70" cy="29" r="7.5" fill={`url(#${uid}_brideGrad)`} />
